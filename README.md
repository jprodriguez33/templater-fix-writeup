...
Affected software: Templater — a template plugin for Obsidian
...
A .js file containing harmful code in the configured user-scripts folder.
A settings file (data.json) pointing Templater at that folder.
A note containing text like tp.user.evil. somewhere in it.
...
jsconst wrapping_fn = window.eval(
...
as poc.js.
In any note, type tp.user.evil.
Click right after the . and type or delete one character.
Check your home directory — templater-poc-fired.txt will be present,
...
Apply the same confirmation-popup treatment to the user-scripts-folder
...
confirmation.
Stop executing script files to build autocomplete suggestions — parse the
